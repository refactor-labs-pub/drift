...
fn count_parameters(node: Node, lang: Language) -> usize {
    let params_node = match lang {
        Language::Python => node.child_by_field_name("parameters"),
        Language::Java => node.child_by_field_name("parameters"),
        Language::TypeScript | Language::JavaScript => node.child_by_field_name("parameters"),
    };
